...
interface Withdrawal {
  ...
  created_at: string;
  updated_at: string;
  influencer?: { name: string; email: string; coupon_code: string };
}
...
export default function InfluencerWithdrawals() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const res = await callApi('influencer-withdrawals-admin', 'GET', {});
  ...
  const submitAction = async () => {
    ...
    const res = await callApi(`influencer-withdrawals/${acting.id}`, 'PUT', body);
    setSubmitting(false);
    if (res.success) {
  ...
  const filtered = withdrawals.filter(w => {
    ...
    if (search) {
      ...
      return (
        w.influencer?.name?.toLowerCase().includes(q) ||
        w.influencer?.email?.toLowerCase().includes(q) ||
        w.influencer?.coupon_code?.toLowerCase().includes(q) ||
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search influencer name, email, coupon, account…"
            value={search}
                  ...
                  <>
                    <tr key={w.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3">
                        <p className="font-medium text-gray-900">{w.influencer?.name || '—'}</p>
                        <p className="text-xs text-gray-400">{w.influencer?.email || ''} {w.influencer?.coupon_code ? `· ${w.influencer.coupon_code}` : ''}</p>
                      </td>
                      <td className="px-4 py-3 text-right font-bold text-gray-900">{fmt(w.amount)}</td>
                      <td className="px-4 py-3">
                        <p className="font-mono text-xs text-gray-700">{w.account_number || '—'}</p>
                        <p className="text-xs text-gray-400">{w.bank_name} · {w.account_name}</p>
                      </td>
                      <td className="px-4 py-3">
          ...
          <div className="sm:hidden space-y-3">
            {filtered.map(w => (
              <div key={w.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                {/* Top row: influencer + amount */}
                <div className="flex items-start justify-between px-4 pt-4 pb-2 gap-3">
                  <div className="min-w-0">
                    <p className="font-semibold text-gray-900 leading-snug">{w.influencer?.name || '—'}</p>
                    <p className="text-xs text-gray-400 truncate">{w.influencer?.email || ''}</p>
                  </div>
                ...
                <div className="px-4 pb-3 border-b border-gray-100">
                  <p className="font-mono text-xs text-gray-700">{w.account_number || '—'}</p>
                  <p className="text-xs text-gray-400">{[w.bank_name, w.account_name].filter(Boolean).join(' · ') || '—'}</p>
