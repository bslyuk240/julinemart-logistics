...
interface Withdrawal {
  ...
  created_at: string;
  updated_at: string;
  rider?: { full_name: string; email: string; phone?: string };
}
...
export default function RiderWithdrawals() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !payload.success) throw new Error(payload?.error || 'Failed to load withdrawals');
  ...
  const submitAction = async () => {
    ...
    try {
      ...
      notification.success(`Withdrawal ${acting.action === 'paid' ? 'marked as paid' : acting.action + 'd'}`);
  ...
  return (
    <div className="w-full max-w-none p-4 sm:p-6 space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Rider Withdrawals</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search rider name, email, account…"
            value={search}
                  ...
                  <>
                    <tr key={w.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3">
                        <p className="font-medium text-gray-900">{w.rider?.full_name || '—'}</p>
                        <p className="text-xs text-gray-400">{w.rider?.email || ''}</p>
                      </td>
          ...
          <div className="sm:hidden space-y-3">
            {filtered.map(w => (
              <div key={w.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                {/* Top row: rider + amount */}
                <div className="flex items-start justify-between px-4 pt-4 pb-2 gap-3">
                  <div className="min-w-0">
                    <p className="font-semibold text-gray-900 leading-snug">{w.rider?.full_name || '—'}</p>
                    <p className="text-xs text-gray-400 truncate">{w.rider?.email || ''}</p>
                  </div>
