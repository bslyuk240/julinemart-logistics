/**
 * Admin / staff: gift box reviews — filter, approve, reject.
 */
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${functionsBase}/admin-gift-box-reviews?${params}`,
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Gift box reviews</h1>
          <p className="text-sm text-gray-600 mt-1">
            Approve or reject customer reviews. Approved reviews appear on the gift box page.
          </p>
        </div>
              ...
              <div className="flex flex-wrap items-start justify-between gap-3 mb-2">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    {new Date(r.created_at).toLocaleString()} · Box:{' '}
                    <span className="text-gray-600">{r.gift_boxes?.name ?? r.gift_box_id}</span>
                  </p>
                </div>
                <div className="flex items-center gap-2">
...
export default GiftBoxReviewsPage;
