...
export default function MobileInfluencerWithdrawals() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const res = await vendorFetch<{ success?: boolean; data?: Withdrawal[] }>('influencer-withdrawals-admin', 'GET', {});
  ...
  const submitAction = async () => {
    ...
    const res = await vendorFetch<{ success?: boolean; error?: string }>(`influencer-withdrawals/${acting.id}`, 'PUT', body);
    setSubmitting(false);
    if (res.success) {
      ...
      notification.error('Action failed', res.error || 'Unable to update withdrawal');
    }
  };
    ...
    <>
      <PullToRefresh onRefresh={load}>
        <div className="space-y-3 p-4" style={{ paddingBottom: TABBAR_SPACE }}>
          <div>
            <h1 className="text-lg font-bold text-gray-900">Influencer Payouts</h1>
            <p className="text-xs text-gray-500">Withdrawal requests from influencers</p>
          </div>
          ...
          <div className="flex gap-2">
            <div className="flex min-w-0 flex-1 items-center gap-2 rounded-xl bg-white px-3 py-2 ring-1 ring-gray-100">
              ...
              <input
                ...
                placeholder="Search influencer or account…"
                ...
                <div key={w.id} className="rounded-xl bg-white ring-1 ring-gray-100">
                  <div className="flex items-start justify-between gap-3 p-3">
                    <div className="min-w-0">
                      <p className="font-semibold text-gray-900">{w.influencer?.name || '—'}</p>
                      <p className="truncate text-xs text-gray-400">{w.influencer?.email}</p>
                      <p className="mt-1 font-mono text-xs text-gray-600">{w.account_number || '—'}</p>
                      <p className="text-xs text-gray-400">
                        {[w.bank_name, w.account_name].filter(Boolean).join(' · ')}
                      </p>
                    </div>
