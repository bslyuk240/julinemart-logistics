...
| 9 | Delivery Exceptions (structured incident types, distinct workflow actions) | 🟡 | "Report Problem" on Active Delivery logs one of the brief's 10 incident types + an optional note to `tracking_events` (new `report_problem` action on `rider-jobs.js`), visible to staff, does not change shipment status. **Not built:** each reason type driving a distinct next step (return required, reassign, etc.) — that's real workflow logic, still its own piece. |
