...
export default function MobileVendorWithdrawals() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const res = await vendorFetch<{ success?: boolean; data?: Withdrawal[] }>('vendor-withdrawals-admin', 'GET', {});
  ...
  const submitAction = async () => {
    ...
    const res = await vendorFetch<{ success?: boolean; error?: string }>(`vendor-withdrawals/${acting.id}`, 'PUT', body);
    setSubmitting(false);
    if (res.success) {
    ...
    <>
      <PullToRefresh onRefresh={load}>
        <div className="space-y-3 p-4" style={{ paddingBottom: TABBAR_SPACE }}>
          <div>
            <h1 className="text-lg font-bold text-gray-900">Vendor Payouts</h1>
            <p className="text-xs text-gray-500">Withdrawal requests from vendors</p>
          </div>
                ...
                <div key={w.id} className="rounded-xl bg-white ring-1 ring-gray-100">
                  <div className="flex items-start justify-between gap-3 p-3">
                    <div className="min-w-0">
                      <p className="font-semibold text-gray-900">{w.vendor?.store_name || '—'}</p>
                      <p className="truncate text-xs text-gray-400">{w.vendor?.email}</p>
