...
import { Store, Mail, CheckCircle, XCircle, Clock, ChevronDown, ChevronUp, Eye, Send, AlertTriangle, Pencil, X, Check, Trash2 } from 'lucide-react';
    ...
    <div className="w-full max-w-none p-4 sm:p-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between mb-6">
        <div className="flex items-center gap-3">
          <Store className="w-6 h-6 text-purple-600 shrink-0" />
        ...
        <div>
          {placeholderCount > 0 && (
            <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-4 text-sm text-amber-800">
              ...
              <span>
                <strong>{placeholderCount} vendor{placeholderCount > 1 ? 's' : ''}</strong> still have placeholder emails.
                Tap the <Pencil className="w-3 h-3 inline" /> icon to update before sending an invite.
              </span>
            </div>
          )}
            ...
            <>
                    ...
                    {vendors.map(v => {
                      ...
                      return (
                        <tr key={v.id} className={`hover:bg-gray-50 ${placeholder ? 'bg-amber-50/40' : ''}`}>
                          ...
                          <td className="px-4 py-3">
                            <EmailCell v={v} editingEmail={editingEmail} emailDraft={emailDraft} setEmailDraft={setEmailDraft} savingEmail={savingEmail} emailMsg={emailMsg} startEditEmail={startEditEmail} cancelEditEmail={cancelEditEmail} saveEmail={saveEmail} />
                          </td>
                          ...
                          <td className="px-4 py-3 text-center">
                            ...
                            <VendorActionButton v={v} placeholder={placeholder} inviting={inviting} startEditEmail={startEditEmail} handleInvite={handleInvite} />
                          </td>
                        </tr>
                      );
                    })}
              ...
              <div className="sm:hidden space-y-2">
                {vendors.map(v => {
                  const placeholder = isPlaceholderEmail(v.email);
                  return (
                    <div key={v.id} className={`bg-white rounded-xl border border-gray-200 p-4 ${placeholder ? 'border-amber-200 bg-amber-50/30' : ''}`}>
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <div className="min-w-0">
                          <button
                            onClick={() => navigate(`/admin/vendors/${v.id}`)}
                            className="font-semibold text-gray-900 hover:text-purple-700 text-left leading-tight"
                          >
                            {v.store_name}
                          </button>
                          <div className="mt-1">
                            {placeholder ? (
                              <span className="flex items-center gap-1 text-amber-600 text-xs font-medium">
                                <AlertTriangle className="w-3 h-3" /> Placeholder email
                              </span>
                            ) : (
                        ...
                        <div className="flex flex-col items-end gap-1.5 shrink-0">
                          ...
                          {v.user_id ? (
                            ...
                            <span className="inline-flex items-center gap-1 text-xs text-gray-400">
                              <XCircle className="w-3 h-3" /> No portal
                            </span>
                          )}
                          <span className="text-xs text-gray-400">{v.commission_rate}% commission</span>
                        </div>
                      </div>

                      {/* Inline email edit on mobile */}
                      {editingEmail === v.id && (
                        <div className="flex flex-col gap-1 mb-2">
                          <div className="flex items-center gap-1">
                            <input
                              ...
                              autoFocus
                              className="flex-1 border border-purple-300 rounded px-2 py-1.5 text-sm focus:ring-2 focus:ring-purple-300 outline-none"
                            />
                            <button onClick={() => saveEmail(v.id)} disabled={savingEmail} className="p-1.5 bg-green-600 hover:bg-green-700 text-white rounded disabled:opacity-50">
                              <Check className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={cancelEditEmail} className="p-1.5 bg-gray-200 hover:bg-gray-300 text-gray-600 rounded">
                              <X className="w-3.5 h-3.5" />
                            </button>
                          </div>
                          {emailMsg?.id === v.id && (
                            <p className={`text-xs ${emailMsg.ok ? 'text-green-600' : 'text-red-500'}`}>{emailMsg.msg}</p>
                          )}
                        </div>
                      )}
                      ...
                      <div className="flex items-center gap-2 flex-wrap mt-1">
                        {inviteMsg?.id === v.id && (
                          <p className={`text-xs w-full ${inviteMsg.ok ? 'text-green-600' : 'text-red-500'}`}>{inviteMsg.msg}</p>
                        )}
                        <VendorActionButton v={v} placeholder={placeholder} inviting={inviting} startEditEmail={startEditEmail} handleInvite={handleInvite} />
                        {!placeholder && v.email && (
                          <button
                            onClick={() => startEditEmail(v)}
                            className="inline-flex items-center gap-1 text-xs text-gray-400 hover:text-purple-600 px-2 py-1.5 rounded-lg border border-gray-200 hover:border-purple-300 transition"
                          >
            ...
            <div className="space-y-3">
              {apps.map(app => (
                <div key={app.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                  {/* Header row */}
                  <div
                    className="flex items-start justify-between px-4 py-4 cursor-pointer hover:bg-gray-50 gap-3"
                    onClick={() => setExpandedApp(expandedApp === app.id ? null : app.id)}
                  >
                    <div className="min-w-0">
                      <p className="font-semibold text-gray-900 leading-snug">{app.store_name}</p>
                      <p className="text-sm text-gray-500 truncate">{app.full_name} · {app.email}</p>
                    </div>
                      ...
                      {app.status === 'pending' && (
                        <div className="flex flex-col sm:flex-row items-start gap-3">
                          <button
                            ...
                            className="w-full sm:w-auto flex items-center justify-center gap-1.5 bg-green-600 hover:bg-green-700 text-white text-sm px-4 py-2.5 rounded-lg disabled:opacity-50 transition"
                          >
                          ...
                          {showRejectInput !== app.id ? (
                            <button
                              ...
                              className="w-full sm:w-auto flex items-center justify-center gap-1.5 bg-red-50 hover:bg-red-100 text-red-600 text-sm px-4 py-2.5 rounded-lg border border-red-200 transition"
                            >
                            ...
                            <div className="flex-1 w-full flex flex-col sm:flex-row gap-2">
                              ...
                              <div className="flex gap-2">
                                <button
                                  ...
                                  disabled={actioning === app.id}
                                  className="flex-1 sm:flex-none bg-red-600 hover:bg-red-700 text-white text-sm px-4 py-2 rounded-lg disabled:opacity-50 transition"
                                >
                                  {actioning === app.id ? 'Rejecting…' : 'Confirm'}
                                </button>
...
function EmailCell({ v, editingEmail, emailDraft, setEmailDraft, savingEmail, emailMsg, startEditEmail, cancelEditEmail, saveEmail }: {
  v: Vendor;
  editingEmail: string | null;
  emailDraft: string;
  setEmailDraft: (s: string) => void;
  savingEmail: boolean;
  emailMsg: { id: string; msg: string; ok: boolean } | null;
  startEditEmail: (v: Vendor) => void;
  cancelEditEmail: () => void;
  saveEmail: (id: string) => void;
}) {
  const placeholder = isPlaceholderEmail(v.email);
  if (editingEmail === v.id) {
    return (
      <div className="flex flex-col gap-1">
        <div className="flex items-center gap-1">
          ...
          />
          <button onClick={() => saveEmail(v.id)} disabled={savingEmail} className="p-1 bg-green-600 hover:bg-green-700 text-white rounded disabled:opacity-50" title="Save">
            <Check className="w-3 h-3" />
          </button>
          <button onClick={cancelEditEmail} className="p-1 bg-gray-200 hover:bg-gray-300 text-gray-600 rounded" title="Cancel">
            <X className="w-3 h-3" />
          </button>
        </div>
        {emailMsg?.id === v.id && (
  ...
  return (
    <div className="flex items-center gap-1.5 group">
      ...
      {emailMsg?.id === v.id && !editingEmail && (
        <span className={`text-xs ml-1 ${emailMsg.ok ? 'text-green-600' : 'text-red-500'}`}>{emailMsg.msg}</span>
      )}
    </div>
  );
}

function VendorActionButton({ v, placeholder, inviting, startEditEmail, handleInvite }: {
  v: Vendor;
  placeholder: boolean;
